...
scripts/config --enable CONFIG_RWMMIO_TRACING
...
if grep -Fq "systemd.run=" "$CMDLINE_FILE"; then
    ...
    echo "$(cat "$CMDLINE_FILE") initrd=initramfs.cpio systemd.run=/boot/firstrun.sh systemd.run_success_action=reboot systemd.unit=kernel-command-line.target" > "$CMDLINE_FILE"
fi
