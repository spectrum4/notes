...
# An instruction fetch on address 0x0008 may page in a peripheral ROM
...
#// ERROR-1
# L0008:  LD      HL,(0x5C5D)             // fetch the character address from CH_ADD.
...
X0013:  .byte    0xFF                     // this byte is used by the SPECTRUM command in
...
#// GET-CHAR
L0018:  LD      HL,(0x5C5D)               // fetch the address from CH_ADD.
...
#// ERROR-3
# L0055:  LD      (IY+0x00),L             // store it in the system variable ERR_NR.
...
#// CH-ADD+1
L0074:  LD      HL,(0x5C5D)               // fetch address from CH_ADD.

#// TEMP-PTR1
L0077:  INC     HL                        // increase the character address by one.

#// TEMP-PTR2
L0078:  LD      (0x5C5D),HL               // update CH_ADD with character address.
...
#// SKIP-OVER
L007D:  CP      0x21                      // test if higher than space.
...
#// KEY-SCAN
L028E:  LD      L,0x2F                    // initial key value
...
#// KEY-BITS
L02A1:  SUB     0x08                      // subtract 8 from the key value
...
#// K-ST-LOOP
L02C6:  BIT     7,(HL)                    // is it free ?  (0xFF)
...
L02F1:  LD      E,A                       // store key in E
        ...
        LD      A,(0x5C09)                // pick up system variable REPDEL
...
#// K-END
L0308:  LD      (0x5C08),A                // update LASTK system variable.
        SET     5,(IY+0x01)               // update FLAGS  - signal new key.
        RET                               // done
...
#// K-E-LET
L0341:  LD      HL,L022C-0x41             // base address of E-UNSHIFT L022c
                                          // ( 0x01EB in standard ROM )
        INC     B                         // test B is it empty i.e. not a shift
        JR      Z,L034A                   // forward to K-LOOK-UP if neither shift

        LD      HL,L0246-0x41             // Address: 0x0205 L0246-0x41 EXT-SHIFT base

#// K-LOOK-UP
L034A:  LD      D,0x00                    // prepare to index
...
#// K-KLC-LET
L034F:  LD      HL,L026A-0x41             // prepare base of sym-codes
...
#// K-TOKENS
L0364:  ADD     A,0xA5                    // add offset to main code so that 'A'
                                          // becomes 'NEW' etc.
        RET                               // return
...
#// K-DIGIT
L0367:  CP      0x30                      // is it '0' or higher ?
...
# continue with extended digits 0-9.
        ...
        LD      HL,L0284-0x30             // 0x0254 - base of E-DIGITS
...
#// K-8-&-9
L0382:  SUB     0x36                      // reduce to 02 and 03  bright codes
...
#// K-GRA-DGT
L0389:  LD      HL,L0260-0x30             // 0x0230 base address of CTL-CODES

        CP      0x39                      // is key '9' ?
        JR      Z,L034A                   // back to K-LOOK-UP - changed to 0x0F, GRAPHICS.

        CP      0x30                      // is key '0' ?
        JR      Z,L034A                   // back to K-LOOK-UP - changed to 0x0C, delete.
...
L039D:  INC     B                         // return with digit codes if neither
        ...
        LD      HL,L0260-0x30             // prepare base of table CTL-CODES.
...
#// K-@-CHAR
L03B2:  LD      A,0x40                    // substitute ASCII '@'
        RET                               // return.
...
L0425:  LD      B,0xFA                    // 6 octaves below middle C
...
L046E:  .byte    0x89, 0x02, 0xD0, 0x12, 0x86//  261.625565290         C
...
# the outer loop is counted by H
        ...
        JP      P,L04D8                   // back to SA-LEADER until H becomes 0xFF
...
#// SA-OUT
L051C:  OUT     (0xFE),A                  // blue and mic on OR  yellow and mic off.
...
#// LD-LEADER
L0580:  LD      B,0x9C                    // set timing value.
        CALL    L05E3                     // routine LD-EDGE-2
...
#// LD-SYNC
L058F:  LD      B,0xC9                    // initial timing value in B.
...
#// LD-VERIFY
L05BD:  LD      A,(IX+0x00)               // fetch byte from memory.
...
#// LD-MARKER
L05C8:  LD      L,0x01                    // initialize as %00000001
...
#// LD-EDGE-1
L05E7:  LD      A,0x16                    // a delay value of twenty two.
...
# Note. For ZASM use SUB 0xE0 as next instruction.
...
#// SA-V-NEW
L0685:  LD      (IX+0x0E),C               // place character array name in  header.
...
#// SA-V-TYPE
L068F:  LD      (IX+0x00),A               // place type 0, 1 or 2 in descriptor.
...
#// SA-SCR$
L06A0:  CP      0xAA                      // is character the token 'SCREEN$' ?
...
#// SA-CODE
L06C3:  CP      0xAF                      // is character the token 'CODE' ?
...
#// SA-TYPE-3
L0710:  LD      (IX+0x00),0x03            // place type 3 - code in descriptor.
...
#// SA-LINE
L0716:  CP      0xCA                      // is character the token 'LINE' ?
...
#// SA-LINE-1
L0723:  LD      A,(0x5C74)                // fetch command from T_ADDR
        AND     A                         // test for SAVE.
...
#// SA-TYPE-0
L073A:  LD      (IX+0x00),0x00            // place type zero - program in descriptor.
...
#// SA-ALL
L075A:  LD      A,(0x5C74)                // fetch command from T_ADDR
        AND     A                         // test for zero - SAVE.
...
#// LD-TYPE
L078A:  CP      0x04                      // check if type in acceptable range 0 - 3.
...
#// VR-CONT-3
L0800:  LD      A,0xFF                    // signal data block to be loaded
...
#// LD-CONTRL
L0808:  LD      E,(IX+0x0B)               // fetch length of found data block
...
#// LD-CONT-1
L0819:  LD      L,(IX-0x06)               // fetch length from
...
#// LD-CONT-2
L0825:  LD      DE,0x0005                 // allow overhead of five bytes.
...
#// LD-DATA-1
L084C:  LD      HL,(0x5C59)               // address E_LINE
...
L0873:  EX      DE,HL                     // transfer dest to DE.
        ...
        LD      (0x5C42),HL               // set sytem variable to line number NEWPPC
...
#// ME-CONTRL
L08B6:  LD      C,(IX+0x0B)               // fetch length
...
X08CE:  LD      DE,(0x5C53)               // set DE from system variable PROG.
...
#// ME-OLD-V2
L0909:  AND     0xE0                      // keep bits 11100000
...
#// ME-VAR-L1
L0921:  LD      A,0xFF                    // indicate a replacement.
...
#
# //; ctlchrtab
# L0A11:  .byte    L0A5F - $              // 06d offset 0x4E to Address: PO-COMMA
#         .byte    L0A69 - $              // 07d offset 0x57 to Address: PO-QUEST
#         .byte    L0A23 - $              // 08d offset 0x10 to Address: PO-BACK-1
#         .byte    L0A3D - $              // 09d offset 0x29 to Address: PO-RIGHT
#         .byte    L0A69 - $              // 10d offset 0x54 to Address: PO-QUEST
#         .byte    L0A69 - $              // 11d offset 0x53 to Address: PO-QUEST
#         .byte    L0A69 - $              // 12d offset 0x52 to Address: PO-QUEST
#         .byte    L0A4F - $              // 13d offset 0x37 to Address: PO-ENTER
#         .byte    L0A69 - $              // 14d offset 0x50 to Address: PO-QUEST
#         .byte    L0A69 - $              // 15d offset 0x4F to Address: PO-QUEST
#         .byte    L0A7A - $              // 16d offset 0x5F to Address: PO-1-OPER
#         .byte    L0A7A - $              // 17d offset 0x5E to Address: PO-1-OPER
#         .byte    L0A7A - $              // 18d offset 0x5D to Address: PO-1-OPER
#         .byte    L0A7A - $              // 19d offset 0x5C to Address: PO-1-OPER
#         .byte    L0A7A - $              // 20d offset 0x5B to Address: PO-1-OPER
#         .byte    L0A7A - $              // 21d offset 0x5A to Address: PO-1-OPER
#         .byte    L0A75 - $              // 22d offset 0x54 to Address: PO-2-OPER
#         .byte    L0A75 - $              // 23d offset 0x53 to Address: PO-2-OPER
#
#
# // -------------------
# // Cursor left routine
...
#
# //; PO-BACK-2
# L0A38:  LD      C,0x21                  // the leftmost column position.
#
# //; PO-BACK-3
...
#
# //; PO-RIGHT
# L0A3D:  LD      A,(0x5C91)              // fetch P_FLAG value
...
#
# //; PO-ENTER
# L0A4F:  BIT     1,(IY+0x01)             // test FLAGS  - is printer in use ?
...
#         AND     0x1F                    // if (curx <= newx) { A = newx - curx }
...
#
# //; PO-FETCH
# L0B03:  BIT     1,(IY+0x01)             // test FLAGS  - Is printer in use
#         JR      NZ,L0B1D                // to PO-F-PR if so
#
#                                         // assume upper screen
#         LD      BC,(0x5C88)             // S_POSN
#         LD      HL,(0x5C84)             // DF_CC display file address
#         BIT     0,(IY+0x02)             // TV_FLAG  - Lower screen in use ?
...
#
# //; PO-GR-1
# L0B38:  LD      HL,0x5C92               // address MEM-0 - a temporary buffer in
...
#// PO-SCR
L0C55:  BIT     1,(IY+0x01)               // test FLAGS  - is printer in use ?
...
L0CD2:  CALL    L0DFE                     // routine CL-SC-ALL to scroll whole display
        ...
        LD      C,0x21                    // set C to 0x21 (was 0x21 from above routine)
...
#// PO-SCR-4
L0D02:  CP      0x02                      // is line number less than 2 ?
...
L0D1C:  PUSH    AF                        // save scroll number.
        ...
        LD      B,0x18                    // set count to whole display ??
                                          // Note. should be 0x17 and the top line
...
#
# //; TEMPS-1
# L0D5B:  LD      (0x5C8F),HL             // transfer values to ATTR_T and MASK_T
#
...
# //; CLS-LOWER
# L0D6E:  LD      HL,0x5C3C               // address System Variable TV_FLAG.
...
#         LD      B,(IY+0x31)             // fetch lower screen DF_SZ
#         CALL    L0E44                   // routine CL-LINE clears lower part
#                                         // and sets permanent attributes.
#
#         LD      HL,0x5AC0               // fetch attribute address leftmost cell,
#                                         // second line up.
...
#
# //; CLS-1
# L0D87:  LD      C,0x20                  // set counter to 32 characters per line
#
# //; CLS-2
...
#
# //; CL-ALL
# L0DAF:  LD      HL,0x0000               // initialize plot coordinates.
#         LD      (0x5C7D),HL             // set COORDS to 0,0.
...
#
# //; CL-SET
# L0DD9:  LD      HL,0x5B00               // the base address of printer buffer
#         BIT     1,(IY+0x01)             // test FLAGS  - is printer in use ?
...
#
# //; CL-SET-2
# L0DF4:  LD      A,0x21                  // the column 0x1-0x21 is reversed
#         SUB     C                       // to range 0x00 - 0x20
...
#// CL-SC-ALL
L0DFE:  LD      B,0x17                    // scroll 23 lines, after 'scroll?'.
...
# ( so HL = 0x4800 or 0x5000 ) ( but also sometimes 0x4000 )

#// CL-SCR-2
L0E0D:  EX      DE,HL                     // copy HL to DE.
        LD      HL,0xF8E0                 // subtract 0x08 from H and add 0xE0 to L -
...
L0E19:  EX      DE,HL                     // save source in DE.
        ...
        LD      C,A                       // transfer byte count to C (0xE0 at most)
...
#         AND     0xF8                    // will be 16, 8 or 0  (AND 0x18 will do).
...
#         POP     BC                      // restore the line 0x01-0x24.
#         LD      C,0x21                  // make column 0x21. (No use is made of this)
...
#// CL-ATTR
L0E88:  LD      A,H                       // fetch H to A - 0x48, 0x50, or 0x58.
        RRCA                              // divide by
        RRCA                              // eight.
        RRCA                              // 0x09, 0x0A or 0x0B.
        DEC     A                         // 0x08, 0x09 or 0x0A.
        OR      0x50                      // 0x58, 0x59 or 0x5A.
...
L0EAF:  LD      HL,0x4000                 // address start of the display file.
...
#// COPY-END
L0EDA:  LD      A,0x04                    // output value 4 to port
...
#// CLEAR-PRB
L0EDF:  LD      HL,0x5B00                 // the location of the buffer.
...
#// COPY-L-2
L0F0C:  IN      A,(0xFB)                  // test now to see if
...
#// EDITOR
L0F2C:  LD      HL,(0x5C3D)               // fetch ERR_SP
        PUSH    HL                        // save on stack
...
#// ADD-CHAR
L0F81:  RES     0,(IY+0x07)               // set MODE to 'L'

X0F85:  LD      HL,(0x5C5B)               // fetch address of keyboard cursor from K_CUR
...
L0F92:  LD      E,A                       // character to E.
        ...
        LD      HL,L0FA0 - 7              // base address of editing keys table. 0x0F99
        ADD     HL,DE                     // add E
...
#// ed-keys-t
L0FA0:  .byte    L0FA9 - $                // 07d offset 0x09 to Address: ED-EDIT
        .byte    L1007 - $                // 08d offset 0x66 to Address: ED-LEFT
        .byte    L100C - $                // 09d offset 0x6A to Address: ED-RIGHT
        .byte    L0FF3 - $                // 10d offset 0x50 to Address: ED-DOWN
        .byte    L1059 - $                // 11d offset 0xB5 to Address: ED-UP
        .byte    L1015 - $                // 12d offset 0x70 to Address: ED-DELETE
        .byte    L1024 - $                // 13d offset 0x7E to Address: ED-ENTER
        .byte    L1076 - $                // 14d offset 0xCF to Address: ED-SYMBOL
        .byte    L107C - $                // 15d offset 0xD4 to Address: ED-GRAPH
...
#// ED-EDIT
L0FA9:  LD      HL,(0x5C49)               // fetch E_PPC the last line number entered.
...
#// ED-DOWN
L0FF3:  BIT     5,(IY+0x37)               // test FLAGX  - Input Mode ?
...
#// ED-STOP
L1001:  LD      (IY+0x00),0x10            // set ERR_NR to 'STOP in INPUT' code
...
#// ED-CUR
L1011:  LD      (0x5C5B),HL               // update K_CUR system variable
        RET                               // return to ED-LOOP
...
L1026:  POP     HL                        // the previous value of ERR_SP
        ...
        BIT     7,(IY+0x00)               // is ERR_NR 0xFF (= 'OK') ?
        RET     NZ                        // return if so
...
#// ED-UP
L1059:  BIT     5,(IY+0x37)               // test FLAGX  - input mode ?
...
# This is chr$ 14.
#// ED-SYMBOL
L1076:  BIT     7,(IY+0x37)               // test FLAGX - is this INPUT LINE ?
...
#// ED-ERROR
L107F:  BIT     4,(IY+0x30)               // test FLAGS2  - is K channel in use ?
...
#// KEY-INPUT
L10A8:  BIT     3,(IY+0x02)               // test TV_FLAG  - has a key been pressed in
                                          // editor ?
...
#// KEY-MODE
L10E6:  CP      0x0E                      // compare with chr 14d
...
#// KEY-FLAG
L10F4:  SET     3,(IY+0x02)               // update TV_FLAG  - show key state has changed
...
#// KEY-DATA
L1105:  LD      (IY-0x2D),C               // put the colour (0-7)/state(0/1) in KDATA
...
#// KEY-NEXT
L110D:  LD      A,(0x5C0D)                // pick up the parameter stored in KDATA.
...
#// KEY-CHAN
L1113:  LD      HL,(0x5C4F)               // address start of CHANNELS area using CHANS
                                          // system variable.
...
#// ED-BLANK
L1150:  LD      A,(0x5C8B)                // fetch SPOSNL_hi is current line
...
#// ED-SPACES
L115E:  LD      A,0x20                    // prepare a space.
        ...
        CALL    L09F4                     // routine PRINT-OUT prints a space over
                                          ...
                                          // using 0x09F4 to print to the lower screen,
...
#// ED-FULL
L1167:  LD      D,0x00                    // prepare to moan.
        LD      E,(IY-0x02)               // fetch RASP value.
        LD      HL,0x1A90                 // set duration.
...
L117E:  POP     HL                        // restore the old value of ERR_SP.
        ...
        LD      (0x5C82),HL               // and update ECHO_E
...
#// SET-HL
L1190:  LD      HL,(0x5C61)               // fetch WORKSP to HL.
...
#// SET-DE
L1195:  LD      DE,(0x5C59)               // fetch E_LINE to DE
        BIT     5,(IY+0x37)               // test FLAGX  - Input Mode ?
...
L11A7:  LD      A,(HL)                    // fetch character
        ...
        CALL    Z,L19E8                   // routine RECLAIM-2 reclaims space if 0x0E
...
#// RAM-FILL
L11DC:  LD      (HL),0x02                 // load with 2 - red ink on black paper
        DEC     HL                        // next lower
...
L1219:  LD      (0x5CB2),HL               // set system variable RAMTOP to HL.
        ...
        LD      (HL),0x3E                 // top of user ram holds GOSUB end marker
                                          ...
                                          // no significance in the number 0x3E. It has
        ...
        LD      HL,0x0523                 // The keyboard repeat and delay values
        LD      (0x5C09),HL               // are loaded to REPDEL and REPPER.
...
#// MAIN-EXEC
L12A2:  LD      (IY+0x31),0x02            // set DF_SZ lower screen display file
                                          // size to 2 lines.
...
#// MAIN-2
L12AC:  LD      A,0x00                    // select channel 'K' the keyboard
        CALL    L1601                     // routine CHAN-OPEN opens it
        ...
        BIT     7,(IY+0x00)               // test ERR_NR - will be 0xFF if syntax
                                          // is correct.
...
#// MAIN-3
L12CF:  LD      HL,(0x5C59)               // fetch the edit line address from E_LINE.
...
# this must be a direct command.
        ...
        LD      A,0x19                    // compute scroll count to 25 minus
...
L134D:  CALL    L0C0A                     // routine PO-MSG prints them although it would
                                          // be more succinct to use RST 0x10.
        ...
        INC     A                         // test for no error originally 0xFF.
...
#// MAIN-6
L1373:  INC     (IY+0x0D)                 // increment SUBPPC to next statement.

#// MAIN-7
L1376:  LD      BC,0x0003                 // prepare to copy 3 system variables to
...
#// MAIN-9
L1386:  LD      (IY+0x0A),0xFF            // update NSPPC - signal 'no jump'.
...
# is a dummy entry. The last, which begins with 0x7F, the Spectrum
...
L1391:  .byte    0x80
        .byte    'O','K'+0x80             // 0
        .ascii    "NEXT without FO"
        .byte    'R'+0x80                 // 1
        .ascii    "Variable not foun"
        .byte    'd'+0x80                 // 2
        .ascii    "Subscript wron"
        .byte    'g'+0x80                 // 3
        .ascii    "Out of memor"
        .byte    'y'+0x80                 // 4
        .ascii    "Out of scree"
        ...
        .ascii    "Number too bi"
        .byte    'g'+0x80                 // 6
        .ascii    "RETURN without GOSU"
        .byte    'B'+0x80                 // 7
        .ascii    "End of fil"
        .byte    'e'+0x80                 // 8
        .ascii    "STOP statemen"
        .byte    't'+0x80                 // 9
        .ascii    "Invalid argumen"
        .byte    't'+0x80                 // A
        .ascii    "Integer out of rang"
        .byte    'e'+0x80                 // B
        .ascii    "Nonsense in BASI"
        .byte    'C'+0x80                 // C
        .ascii    "BREAK - CONT repeat"
        .byte    's'+0x80                 // D
        .ascii    "Out of DAT"
        .byte    'A'+0x80                 // E
        .ascii    "Invalid file nam"
        .byte    'e'+0x80                 // F
        .ascii    "No room for lin"
        .byte    'e'+0x80                 // G
        .ascii    "STOP in INPU"
        .byte    'T'+0x80                 // H
        .ascii    "FOR without NEX"
        .byte    'T'+0x80                 // I
        .ascii    "Invalid I/O devic"
        .byte    'e'+0x80                 // J
        .ascii    "Invalid colou"
        .byte    'r'+0x80                 // K
        .ascii    "BREAK into progra"
        .byte    'm'+0x80                 // L
        .ascii    "RAMTOP no goo"
        .byte    'd'+0x80                 // M
        .ascii    "Statement los"
        ...
        .ascii    "FN without DE"
        .byte    'F'+0x80                 // P
        .ascii    "Parameter erro"
        .byte    'r'+0x80                 // Q
        .ascii    "Tape loading erro"
        .byte    'r'+0x80                 // R
#// comma-sp
L1537:  .byte    ',',' '+0x80             // used in report line.
#// copyright
L1539:  .byte    0x7F                     // copyright
        .ascii    " 1982 Sinclair Research Lt"
        .byte    'd'+0x80
...
#// REPORT-G
# No Room for line
L1555:  LD      A,0x10                    // i.e. 'G' -0x30 -0x07
...
#// MAIN-ADD
L155D:  LD      (0x5C49),BC               // set E_PPC to extracted line number.
...
L157D:  POP     BC                        // retrieve the length of the new line.
        ...
        DEC     HL
        LD      (HL),D                    // then high byte range 0x0 - 0x27 (1-9999).
...
#// init-chan
L15AF:  .word    L09F4                    // PRINT-OUT
        .word    L10A8                    // KEY-INPUT
        .byte    0x4B                     // 'K'
        .word    L09F4                    // PRINT-OUT
        .word    L15C4                    // REPORT-J
        .byte    0x53                     // 'S'
        .word    L0F81                    // ADD-CHAR
        .word    L15C4                    // REPORT-J
...
L15C4:  RST     08H                       // ERROR-1
        .byte    0x12                     // Error Report: Invalid I/O device


# -------------------
# Initial stream data
# -------------------
# This is the initial stream data for the seven streams 0xFD - 0x03 that is
...
#// init-strm
L15C6:  .byte    0x01, 0x00               // stream 0xFD offset to channel 'K'
        .byte    0x06, 0x00               // stream 0xFE offset to channel 'S'
        .byte    0x0B, 0x00               // stream 0xFF offset to channel 'R'

        .byte    0x01, 0x00               // stream 0x00 offset to channel 'K'
        .byte    0x01, 0x00               // stream 0x01 offset to channel 'K'
        .byte    0x06, 0x00               // stream 0x02 offset to channel 'S'
        .byte    0x10, 0x00               // stream 0x03 offset to channel 'P'
...
#// WAIT-KEY
L15D4:  BIT     5,(IY+0x02)               // test TV_FLAG - clear lower screen ?
...
L15E4:  RST     08H                       // ERROR-1
        .byte    0x07                     // Error Report: End of file
...
# to temporary routines. The normal channel is 0x09F4.

#// OUT-CODE
L15EF:  LD      E,0x30                    // add 48 decimal to give ASCII
...
# // it is entered with a system stream 0xFD - 0xFF, or a user stream 0x00 - 0x0F
# // in the accumulator.
#
# //; CHAN-OPEN
# L1601:  ADD     A,A                     // double the stream (0xFF will become 0xFE etc.)
#         ADD     A,0x16                  // add the offset to stream 0 from 0x5C00
#         LD      L,A                     // result to L
...
#
# //; CHAN-FLAG
# L1615:  LD      (0x5C51),HL             // set CURCHL system variable to the
#                                         // address in HL
...
#
# //; chn-cd-lu
# L162D:  .byte    'K', L1634-0x-1        // offset 0x06 to CHAN-K
#         .byte    'S', L1642-0x-1        // offset 0x12 to CHAN-S
#         .byte    'P', L164D-0x-1        // offset 0x1B to CHAN-P
#
...
#
# //; CHAN-K
# L1634:  SET     0,(IY+0x02)             // update TV_FLAG  - signal lower screen in use
#         RES     5,(IY+0x01)             // update FLAGS    - signal no new key
...
#
# //; CHAN-S
# L1642:  RES     0,(IY+0x02)             // TV_FLAG  - signal main screen in use

# //; CHAN-S-1
# L1646:  RES     1,(IY+0x01)             // update FLAGS  - signal printer not in use
...
#
# //; CHAN-P
# L164D:  SET     1,(IY+0x01)             // update FLAGS  - signal printer in use
#         RET                             // return
...
# in workspace by ADD-CHAR. It is slightly quicker than using a RST 0x30.
...
#// ONE-SPACE
L1652:  LD      BC,0x0001                 // create space for a single character.
...
# command. The program end-marker is the variables end-marker 0x80, or
# if variables exist, then the first character of any variable name.

#// LINE-ZERO
L168F:  .byte    0x00, 0x00               // dummy line number used for direct commands


#// LINE-NO-A
L1691:  EX      DE,HL                     // fetch the previous line to HL and set
        LD      DE,0x168F                 // DE to LINE-ZERO should HL also fail.

# -> The Entry Point.

#// LINE-NO
L1695:  LD      A,(HL)                    // fetch the high byte - max 0x2F
...
#// RESERVE
L169E:  LD      HL,(0x5C63)               // STKBOT first location of calculator stack
...
#// SET-WORK
L16BF:  LD      HL,(0x5C61)               // fetch WORKSP value
...
#// SET-STK
L16C5:  LD      HL,(0x5C63)               // fetch STKBOT value
...
#// REC-EDIT
L16D4:  LD      DE,(0x5C59)               // fetch start of edit line from E_LINE.
...
L16E5:  CALL    L171E                     // routine STR-DATA fetches parameter
        ...
        LD      DE,0xA3E2                 // the number of bytes from stream 4, 0x5C1E,
                                          // to 0x10000
...
# proceed with a negative result.
        ...
        LD      BC,L15C6 + 14             // prepare the address of the byte after
                                          // the initial stream data in ROM. (0x15D4)
...
L1701:  PUSH    HL                        // * save address of stream data pointer
        ...
        LD      C,(HL)                    // pick it up in C.
                                          ...
                                          // get the value 0x10 (the byte preceding 'K').
...
#// cl-str-lu
L1716:  .byte    'K', L171C-0x-1          // offset 5 to CLOSE-STR
        .byte    'S', L171C-0x-1          // offset 3 to CLOSE-STR
        .byte    'P', L171C-0x-1          // offset 1 to CLOSE-STR
...
#// STR-DATA1
L1727:  ADD     A,0x03                    // add the offset for 3 system streams.
...
#// op-str-lu
L177A:  .byte    'K', L1781-0x-1          // 0x06 offset to OPEN-K
        .byte    'S', L1785-0x-1          // 0x08 offset to OPEN-S
        .byte    'P', L1789-0x-1          // 0x0A offset to OPEN-P
...
#// OPEN-K
L1781:  LD      E,0x01                    // 01 is offset to second byte of channel 'K'.
...
#// OPEN-S
L1785:  LD      E,0x06                    // 06 is offset to 2nd byte of channel 'S'
...
#// OPEN-P
L1789:  LD      E,0x10                    // 16d is offset to 2nd byte of channel 'P'
...
# An instruction fetch on 0x0008 may page in a peripheral ROM,
...
#// AUTO-LIST
L1795:  LD      (0x5C3F),SP               // save stack pointer in LIST_SP
        ...
        SET     0,(IY+0x30)               // update FLAGS2  - signal unnecessary to
                                          // clear main screen.
...
#// AUTO-L-2
L17E1:  LD      (0x5C6C),HL               // make S_TOP the same as E_PPC.

# continue here with valid starting point from above or good estimate
# from computation

#// AUTO-L-3
L17E4:  LD      HL,(0x5C6C)               // fetch S_TOP line number to HL.
...
#// LLIST
L17F5:  LD      A,0x03                    // the usual stream for ZX Printer
...
#// LIST
L17F9:  LD      A,0x02                    // default is stream 2 - the upper screen.

#// LIST-1
L17FB:  LD      (IY+0x02),0x00            // the TV_FLAG is initialized with bit 0 reset
...
#// LIST-ALL
L1833:  LD      E,0x01                    // signal current line not yet printed
...
#// OUT-LINE
L1855:  LD      BC,(0x5C49)               // fetch E_PPC the current line which may be
...
#// OUT-LINE1
L1865:  LD      (IY+0x2D),E               // save flag in BREG which is spare.
...
#// OUT-LINE2
L187D:  SET     0,(IY+0x01)               // update FLAGS - suppress leading space.
...
#// OUT-LINE4
L1894:  LD      HL,(0x5C5F)               // fetch X_PTR - possibly the error pointer
                                          // address.
...
#// NUMBER
L18B6:  CP      0x0E                      // character fourteen ?
        RET     NZ                        // return if not.
...
# or the input buffer to the lower screen so a direct call to 0x09F4 can
...
#// OUT-CURS
L18E1:  LD      HL,(0x5C5B)               // fetch K_CUR the current cursor address
        AND     A                         // prepare for true subtraction.
...
#// OUT-C-1
L18F3:  LD      HL,0x5C3B                 // Address FLAGS
...
#// LN-STORE
L191C:  BIT     5,(IY+0x37)               // test FLAGX - input mode ?
        RET     NZ                        // return if so.
...
#// OUT-CH-1
L195A:  CP      0x22                      // is it quote character '"'  ?
...
#// OUT-CH-2
L1968:  SET     2,(IY+0x01)               // update FLAGS - signal L mode if the cursor
                                          // is next.
...
#// EACH-STMT
L198B:  LD      (0x5C5D),HL               // save HL in CH_ADD
...
#// NEXT-O-1
L19C7:  LD      BC,0x0005                 // set BC to 5 for floating point number
...
#// E-LINE-NO
L19FB:  LD      HL,(0x5C59)               // load HL from system variable E_LINE.
...
L1A15:  JP      C,L1C8A                   // to REPORT-C 'Nonsense in BASIC' if over.
                                          ...
                                          // the RST 0x08 will update X_PTR leading to
...
#// P-LET
L1A7A:  .byte    0x01                     // Class-01 - A variable is required.
        .byte    0x3D                     // Separator:  '='
        .byte    0x02                     // Class-02 - An expression, numeric or string,
                                          // must follow.

#// P-GO-TO
L1A7D:  .byte    0x06                     // Class-06 - A numeric expression must follow.
        .byte    0x00                     // Class-00 - No further operands.
        .word    L1E67                    // Address: 0x1E67; Address: GO-TO

#// P-IF
L1A81:  .byte    0x06                     // Class-06 - A numeric expression must follow.
        .byte    0xCB                     // Separator:  'THEN'
        .byte    0x05                     // Class-05 - Variable syntax checked
                                          // by routine.
        .word    L1CF0                    // Address: 0x1CF0; Address: IF

#// P-GO-SUB
L1A86:  .byte    0x06                     // Class-06 - A numeric expression must follow.
        .byte    0x00                     // Class-00 - No further operands.
        .word    L1EED                    // Address: 0x1EED; Address: GO-SUB

#// P-STOP
L1A8A:  .byte    0x00                     // Class-00 - No further operands.
        .word    L1CEE                    // Address: 0x1CEE; Address: STOP

#// P-RETURN
L1A8D:  .byte    0x00                     // Class-00 - No further operands.
        .word    L1F23                    // Address: 0x1F23; Address: RETURN

#// P-FOR
L1A90:  .byte    0x04                     // Class-04 - A single character variable must
                                          // follow.
        .byte    0x3D                     // Separator:  '='
        ...
        .byte    0x05                     // Class-05 - Variable syntax checked
                                          // by routine.
        .word    L1D03                    // Address: 0x1D03; Address: FOR

#// P-NEXT
L1A98:  .byte    0x04                     // Class-04 - A single character variable must
                                          // follow.
        .byte    0x00                     // Class-00 - No further operands.
        .word    L1DAB                    // Address: 0x1DAB; Address: NEXT

#// P-PRINT
L1A9C:  .byte    0x05                     // Class-05 - Variable syntax checked entirely
                                          // by routine.
        .word    L1FCD                    // Address: 0x1FCD; Address: PRINT

#// P-INPUT
L1A9F:  .byte    0x05                     // Class-05 - Variable syntax checked entirely
                                          // by routine.
        .word    L2089                    // Address: 0x2089; Address: INPUT

#// P-DIM
L1AA2:  .byte    0x05                     // Class-05 - Variable syntax checked entirely
                                          // by routine.
        .word    L2C02                    // Address: 0x2C02; Address: DIM

#// P-REM
L1AA5:  .byte    0x05                     // Class-05 - Variable syntax checked entirely
                                          // by routine.
        .word    L1BB2                    // Address: 0x1BB2; Address: REM

#// P-NEW
L1AA8:  .byte    0x00                     // Class-00 - No further operands.
        .word    L11B7                    // Address: 0x11B7; Address: NEW

#// P-RUN
L1AAB:  .byte    0x03                     // Class-03 - A numeric expression may follow
                                          // else default to zero.
        .word    L1EA1                    // Address: 0x1EA1; Address: RUN

#// P-LIST
L1AAE:  .byte    0x05                     // Class-05 - Variable syntax checked entirely
                                          // by routine.
        .word    L17F9                    // Address: 0x17F9; Address: LIST

#// P-POKE
L1AB1:  .byte    0x08                     // Class-08 - Two comma-separated numeric
                                          // expressions required.
        .byte    0x00                     // Class-00 - No further operands.
        .word    L1E80                    // Address: 0x1E80; Address: POKE

#// P-RANDOM
L1AB5:  .byte    0x03                     // Class-03 - A numeric expression may follow
                                          // else default to zero.
        .word    L1E4F                    // Address: 0x1E4F; Address: RANDOMIZE

#// P-CONT
L1AB8:  .byte    0x00                     // Class-00 - No further operands.
        .word    L1E5F                    // Address: 0x1E5F; Address: CONTINUE

#// P-CLEAR
L1ABB:  .byte    0x03                     // Class-03 - A numeric expression may follow
                                          // else default to zero.
        .word    L1EAC                    // Address: 0x1EAC; Address: CLEAR

#// P-CLS
L1ABE:  .byte    0x00                     // Class-00 - No further operands.
        .word    L0D6B                    // Address: 0x0D6B; Address: CLS

#// P-PLOT
L1AC1:  .byte    0x09                     // Class-09 - Two comma-separated numeric
                                          // expressions required with optional colour
                                          // items.
        .byte    0x00                     // Class-00 - No further operands.
        .word    L22DC                    // Address: 0x22DC; Address: PLOT

#// P-PAUSE
L1AC5:  .byte    0x06                     // Class-06 - A numeric expression must follow.
        .byte    0x00                     // Class-00 - No further operands.
        .word    L1F3A                    // Address: 0x1F3A; Address: PAUSE

#// P-READ
L1AC9:  .byte    0x05                     // Class-05 - Variable syntax checked entirely
                                          // by routine.
        .word    L1DED                    // Address: 0x1DED; Address: READ

#// P-DATA
L1ACC:  .byte    0x05                     // Class-05 - Variable syntax checked entirely
                                          // by routine.
        .word    L1E27                    // Address: 0x1E27; Address: DATA

#// P-RESTORE
L1ACF:  .byte    0x03                     // Class-03 - A numeric expression may follow
                                          // else default to zero.
        .word    L1E42                    // Address: 0x1E42; Address: RESTORE

#// P-DRAW
L1AD2:  .byte    0x09                     // Class-09 - Two comma-separated numeric
                                          // expressions required with optional colour
                                          // items.
        .byte    0x05                     // Class-05 - Variable syntax checked
                                          // by routine.
        .word    L2382                    // Address: 0x2382; Address: DRAW

#// P-COPY
L1AD6:  .byte    0x00                     // Class-00 - No further operands.
        .word    L0EAC                    // Address: 0x0EAC; Address: COPY

#// P-LPRINT
L1AD9:  .byte    0x05                     // Class-05 - Variable syntax checked entirely
                                          // by routine.
        .word    L1FC9                    // Address: 0x1FC9; Address: LPRINT

#// P-LLIST
L1ADC:  .byte    0x05                     // Class-05 - Variable syntax checked entirely
                                          // by routine.
        .word    L17F5                    // Address: 0x17F5; Address: LLIST

#// P-SAVE
L1ADF:  .byte    0x0B                     // Class-0B - Offset address converted to tape
                                          // command.

#// P-LOAD
L1AE0:  .byte    0x0B                     // Class-0B - Offset address converted to tape
                                          // command.

#// P-VERIFY
L1AE1:  .byte    0x0B                     // Class-0B - Offset address converted to tape
                                          // command.

#// P-MERGE
L1AE2:  .byte    0x0B                     // Class-0B - Offset address converted to tape
                                          // command.

#// P-BEEP
L1AE3:  .byte    0x08                     // Class-08 - Two comma-separated numeric
                                          // expressions required.
        .byte    0x00                     // Class-00 - No further operands.
        .word    L03F8                    // Address: 0x03F8; Address: BEEP

#// P-CIRCLE
L1AE7:  .byte    0x09                     // Class-09 - Two comma-separated numeric
                                          // expressions required with optional colour
                                          // items.
        .byte    0x05                     // Class-05 - Variable syntax checked
                                          // by routine.
        .word    L2320                    // Address: 0x2320; Address: CIRCLE

#// P-INK
L1AEB:  .byte    0x07                     // Class-07 - Offset address is converted to
                                          // colour code.

#// P-PAPER
L1AEC:  .byte    0x07                     // Class-07 - Offset address is converted to
                                          // colour code.

#// P-FLASH
L1AED:  .byte    0x07                     // Class-07 - Offset address is converted to
                                          // colour code.

#// P-BRIGHT
L1AEE:  .byte    0x07                     // Class-07 - Offset address is converted to
                                          // colour code.

#// P-INVERSE
L1AEF:  .byte    0x07                     // Class-07 - Offset address is converted to
                                          // colour code.

#// P-OVER
L1AF0:  .byte    0x07                     // Class-07 - Offset address is converted to
                                          // colour code.

#// P-OUT
L1AF1:  .byte    0x08                     // Class-08 - Two comma-separated numeric
                                          // expressions required.
        .byte    0x00                     // Class-00 - No further operands.
        .word    L1E7A                    // Address: 0x1E7A; Address: OUT

#// P-BORDER
L1AF5:  .byte    0x06                     // Class-06 - A numeric expression must follow.
        .byte    0x00                     // Class-00 - No further operands.
        .word    L2294                    // Address: 0x2294; Address: BORDER

#// P-DEF-FN
L1AF9:  .byte    0x05                     // Class-05 - Variable syntax checked entirely
                                          // by routine.
        .word    L1F60                    // Address: 0x1F60; Address: DEF-FN

#// P-OPEN
L1AFC:  .byte    0x06                     // Class-06 - A numeric expression must follow.
        .byte    0x2C                     // Separator:  ','          see Footnote *
        .byte    0x0A                     // Class-0A - A string expression must follow.
        .byte    0x00                     // Class-00 - No further operands.
        .word    L1736                    // Address: 0x1736; Address: OPEN

#// P-CLOSE
L1B02:  .byte    0x06                     // Class-06 - A numeric expression must follow.
        .byte    0x00                     // Class-00 - No further operands.
        .word    L16E5                    // Address: 0x16E5; Address: CLOSE

#// P-FORMAT
L1B06:  .byte    0x0A                     // Class-0A - A string expression must follow.
        .byte    0x00                     // Class-00 - No further operands.
        .word    L1793                    // Address: 0x1793; Address: CAT-ETC

#// P-MOVE
L1B0A:  .byte    0x0A                     // Class-0A - A string expression must follow.
        .byte    0x2C                     // Separator:  ','
        .byte    0x0A                     // Class-0A - A string expression must follow.
        .byte    0x00                     // Class-00 - No further operands.
        .word    L1793                    // Address: 0x1793; Address: CAT-ETC
...
#// P-ERASE
L1B10:  .byte    0x0A                     // Class-0A - A string expression must follow.
        .byte    0x00                     // Class-00 - No further operands.
        .word    L1793                    // Address: 0x1793; Address: CAT-ETC

#// P-CAT
L1B14:  .byte    0x00                     // Class-00 - No further operands.
        .word    L1793                    // Address: 0x1793; Address: CAT-ETC
...
#// LINE-SCAN
L1B17:  RES     7,(IY+0x01)               // update FLAGS - signal checking syntax
...
#// SCAN-LOOP
L1B52:  LD      HL,(0x5C74)               // fetch temporary address from T_ADDR
...
#// LINE-RUN
L1B8A:  LD      HL,0xFFFE                 // The dummy value minus two
...
#// LINE-USE
L1BBF:  CP      0x01                      // will set carry if zero.
...
#// NEXT-LINE
L1BD1:  LD      (0x5C55),HL               // store pointer in system variable NXTLIN
        ...
        LD      (IY+0x0A),0xFF            // set statement NSPPC to 0xFF signalling
                                          // no jump to be made.
...
#// VAR-A-1
L1C22:  LD      (IY+0x37),0x00            // set FLAGX to zero
...
#// VAR-A-3
L1C46:  LD      (0x5C72),BC               // update STRLEN
...
#// VAL-FET-1
L1C56:  LD      A,(0x5C3B)                // initial FLAGS to A
...
#// CLASS-07
L1C96:  BIT     7,(IY+0x01)               // test FLAGS - checking syntax only ?
        RES     0,(IY+0x02)               // update TV_FLAG - signal main screen in use
...
L1CA5:  SUB     L1AEB-0xD8 % 256          // convert 0xEB to 0xD8 ('INK') etc.
...
#// FETCH-NUM
L1CDE:  CP      0x0D                      // is character a carriage return ?
...
#// NEXT
L1DAB:  BIT     1,(IY+0x37)               // test FLAGX - handling a new variable ?
...
L1DDA:  RST     28H                       //; FP-CALC
        ...
        .byte    0x02                     //;to L1DE2, NEXT-1 if step negative

        .byte    0x01                     //;exchange         v,l.

#// NEXT-1
L1DE2:  .byte    0x03                     //;subtract         l-v OR v-l.
        .byte    0x37                     //;greater-0        (1/0)
        .byte    0x00                     //;jump-true        .

        .byte    0x04                     //;to L1DE9, NEXT-2 if no more iterations.
...
#// DATA-2
L1E37:  LD      A,0xE4                    // set token to 'DATA' and continue into
...
#// RAND-1
L1E5A:  LD      (0x5C76),BC               // place in SEED system variable.
...
#// CONTINUE
L1E5F:  LD      HL,(0x5C6E)               // fetch OLDPPC line number.
...
#// GO-TO-2
L1E73:  LD      (0x5C42),HL               // save line number in NEWPPC
...
L1EB7:  PUSH    BC                        // save ramtop value.
        ...
        LD      HL,(0x5CB4)               // now P-RAMT (0x7FFF on 16K RAM machine)
...
#// TEST-ROOM
L1F05:  LD      HL,(0x5C65)               // fetch STKEND
        ADD     HL,BC                     // add the supplied test value
        JR      C,L1F15                   // forward to REPORT-4 if over 0xFFFF

        EX      DE,HL                     // was less so transfer to DE
        LD      HL,0x0050                 // test against another 80 bytes
        ADD     HL,DE                     // anyway
        JR      C,L1F15                   // forward to REPORT-4 if this passes 0xFFFF

        SBC     HL,SP                     // if less than the machine stack pointer
        RET     C                         // then return - OK.

#// REPORT-4
L1F15:  LD      L,0x03                    // prepare 'Out of Memory'
        JP      L0055                     // jump back to ERROR-3 at 0x0055
                                          // Note. this error can't be trapped at 0x0008
...
#// free-mem
L1F1A:  LD      BC,0x0000                 // allow no overhead.
...
#// PAUSE-2
L1F49:  BIT     5,(IY+0x01)               // test FLAGS - has a new key been pressed ?
        JR      Z,L1F3D                   // back to PAUSE-1 if not.

#// PAUSE-END
L1F4F:  RES     5,(IY+0x01)               // update FLAGS - signal no new key
        RET                               // and return.
...
#// BREAK-KEY
L1F54:  LD      A,0x7F                    // Input address: 0x7FFE
...
#// DEF-FN-1
L1F6A:  SET      6,(IY+0x01)              // set FLAGS  - Assume numeric result
...
#// DEF-FN-6
L1FA6:  CP      0x29                      // should close with a ')'
...
#// LPRINT
L1FC9:  LD      A,0x03                    // the printer channel
...
#// PRINT
L1FCD:  LD      A,0x02                    // the stream for the upper screen.
...
#// PRINT-4
L1FF2:  CP      0x29                      // return now if this is ')' from input-item.
                                          // (see INPUT.)
...
#// PR-ITEM-2
L200E:  CP      0xAD                      // is character 'TAB' ?
...
#// PR-END-Z
L2045:  CP      0x29                      // is character a ')' ?
        RET     Z                         // return if so -        e.g. INPUT (p$); a$

#// PR-ST-END
L2048:  CP      0x0D                      // is it a carriage return ?
...
#// PR-POSN-2
L2061:  CP      0x27                      // is character a "'" ? (newline)
...
#// INPUT-1
L2096:  LD      (IY+0x02),0x01            // update TV_FLAG - signal lower screen in use
...
#// INPUT-2
L20AD:  LD      (0x5C88),BC               // set S_POSN update upper screen line/column.
...
#// IN-ITEM-2
L20D8:  CP      0xCA                      // is the character the token 'LINE' ?
...
#// IN-PR-3
L2129:  LD      (0x5C5B),HL               // set keyboard cursor K_CUR to HL
        BIT     7,(IY+0x37)               // test FLAGX  - is this INPUT LINE ??
...
#// IN-VAR-2
L2148:  LD      HL,(0x5C61)               // set HL to WORKSP - start of workspace.
...
#// IN-VAR-4
L2161:  LD      (IY+0x22),0x00            // set K_CUR_hi to a low value so that the cursor
...
#// IN-VAR-5
L2174:  LD      HL,0x5C71                 // point HL to FLAGX
...
#// IN-VAR-6
L219B:  LD      HL,(0x5C63)               // STKBOT points to the end of the input.
...
#// IN-ASSIGN
L21B9:  LD      HL,(0x5C61)               // fetch WORKSP start of input
...
#// IN-CHAN-K
L21D6:  LD      HL,(0x5C51)               // fetch address of current channel CURCHL
        INC     HL
        INC     HL                        // advance past
        INC     HL                        // input and
...
#// CO-TEMP-4
L21FC:  SUB     0xC9                      // reduce to control character 0x10 (INK)
                                          // thru 0x15 (OVER).
...
#// CO-TEMP-5
L2211:  SUB     0x11                      // reduce range 0xFF-0x04
...
#// CO-TEMP-9
L2246:  LD      HL,0x5C8F                 // address system variable ATTR_T initially.
...
#// BORDER-1
L22A6:  LD      (0x5C48),A                // update BORDCR with new paper/ink
        RET                               // return.
...
#// PIXEL-ADD
L22AA:  LD      A,0xAF                    // load with 175 decimal.
...
#// PLOT-SUB
L22E5:  LD      (0x5C7D),BC               // store new x/y values in COORDS
...
#// SYNTAX-Z
L2530:  BIT     7,(IY+0x01)               // test FLAGS  - checking syntax only ?
        RET                               // return.
...
#// S-SC-MTCH
L255A:  LD      C,A                       // load C with inverse mask 0x00 or 0xFF
...
#// scan-func
L2596:  .byte    0x22, L25B3-0x-1         // 0x1C offset to S-QUOTE
        .byte    '(', L25E8-0x-1          // 0x4F offset to S-BRACKET
        .byte    '.', L268D-0x-1          // 0xF2 offset to S-DECIMAL
        .byte    '+', L25AF-0x-1          // 0x12 offset to S-U-PLUS

        .byte    0xA8, L25F5-0x-1         // 0x56 offset to S-FN
        .byte    0xA5, L25F8-0x-1         // 0x57 offset to S-RND
        .byte    0xA7, L2627-0x-1         // 0x84 offset to S-PI
        .byte    0xA6, L2634-0x-1         // 0x8F offset to S-INKEY$
        .byte    0xC4, L268D-0x-1         // 0xE6 offset to S-BIN
        .byte    0xAA, L2668-0x-1         // 0xBF offset to S-SCREEN$
        .byte    0xAB, L2672-0x-1         // 0xC7 offset to S-ATTR
        .byte    0xA9, L267B-0x-1         // 0xCE offset to S-POINT
...
#// S-STRING
L25DB:  LD      HL,0x5C3B                 // Address FLAGS system variable.
        RES     6,(HL)                    // signal string result.
...
# ->
#// S-INKEY$
L2634:  LD      BC,0x105A                 // priority 0x10, operation code 0x1A ('read-in')
...
# else read a key from the keyboard.
        ...
        JR      Z,L2665                   // forward to S-INK0x-EN if so

        JP      L3B6C                     // Spectrum 128 patch

L2649:  LD      C,0x00                    // the length of an empty string
        JR      NZ,L2660                  // to S-IK0x-STK to store empty string if
                                          // no key returned.

        CALL    L031E                     // routine K-TEST get main code in A
        JR      NC,L2660                  // to S-IK0x-STK to stack null string if
                                          // invalid
...
#// S-IK0x-STK
L2660:  LD      B,0x00                    // set high byte of length to zero
...
L2668:  CALL    L2522                     // routine S-2-COORD
        CALL    NZ,L2535                  // routine S-SCRN0x-S
...
#// S-NUMERIC
L26C3:  SET     6,(IY+0x01)               // update FLAGS  - Signal numeric result
...
#// S-NEGATE
L26DF:  LD      BC,0x09DB                 // prepare priority 09, operation code 0xC0 +
...
#// S-OPERTR
L2723:  LD      B,0x00                    // prepare to add
        LD      C,A                       // possible operator to C
        LD      HL,L2795                  // Address: 0x2795 - tbl-of-ops
...
# but if found in table the priority has to be looked up.
        ...
        LD      HL,L27B0 - 0xC3           // 0x26ED is base of table
...
#// SF-BRKT-2
L27E4:  CP      0x29                      // is character the closing ')' ?
...
#// SF-FND-DF
L2808:  LD      DE,0x00CE                 // search is for token 'DEF FN' in E,
...
#// LOOK-VARS
L28B2:  SET     6,(IY+0x01)               // update FLAGS - presume numeric result
...
#// V-TEST-FN
L28E3:  LD      A,(0x5C0C)                // load A with DEFADD_hi
...
#// V-RUN
L28FD:  LD      HL,(0x5C4B)               // set HL to start of variables from VARS
...
#// STK-F-ARG
L2951:  LD      HL,(0x5C0B)               // set HL to DEFADD
...
#// SV-CLOSE
L29D8:  CP      0x29                      // as above ')' could follow the expression
...
#// SV-COUNT
L29E7:  LD      HL,0x0000                 // initialize data pointer to zero.
...
L2A20:  RST     08H                       // ERROR-1
        .byte    0x02                     // Error Report: Subscript wrong
...
L2A52:  CALL    L2530                     // routine SYNTAX-Z
        ...
        PUSH    AF                        // save flag (will be 0xFF if parameter>limit)
...
L2A94:  POP     AF                        // pop the running flag.
        ...
        JP      M,L2A20                   // jump back to REPORT-3 if 0xFF.
                                          // 'Subscript out of range'
...
#// STK-STO-$
L2AB2:  RES     6,(IY+0x01)               // update FLAGS - signal string result.
...
# which is returned in BC, setting A to 0xFF, if greater than a limit supplied
...
# is not required and H is set to 0xFF. When checking optional string slice
# parameters, it is entered at the second entry point so as not to disturb
# the running flag A, which may be 0x00 or 0xFF from a previous invocation.
...
#// I-CARRY
L2AE8:  LD      A,D                       // move flag to accumulator 0x00 or 0xFF.
        SBC     A,0x00                    // will set to 0xFF if carry set.
...
#// LET
L2AFF:  LD      HL,(0x5C4D)               // fetch system variable DEST to HL.
...
L2B0C:  INC     HL                        // increase pointer.
        ...
        CP      0x10                      // is it 0x00 - 0x0F ?
        JR      C,L2B29                   // forward to L-SPACES if so.

        CP      0x16                      // is it 0x16 - 0x1F ?
        JR      NC,L2B29                  // forward to L-SPACES if so.

# it was 0x10 - 0x15  so step over a colour code.
...
#// L-SINGLE
L2B4F:  LD      HL,(0x5C4D)               // fetch DEST - HL addresses first character.
        ...
        CALL    L2BEA                     // routine L-FIRST puts A in first character.
                                          ...
                                          // new E_LINE-1  the 0x80 vars end-marker.
...
# the value of variable is deleted but remains after calculator stack.
        ...
        RST     28H                       //; FP-CALC
        .byte    0x02                     //;delete      ; delete variable value
        .byte    0x38                     //;end-calc
...
#// L-EXISTS
L2B66:  BIT     6,(IY+0x01)               // test FLAGS - numeric or string result ?
...
#// L-DELETE$
L2B72:  LD      HL,(0x5C4D)               // fetch DEST to HL.
...
#// L-NEW$
L2BC0:  LD      A,0xDF                    // indicator mask %11011111 for
...
L2C15:  JR      C,L2C1F                   // skip to D-LETTER if variable did not exist.
        ...
        CALL    L19B8                     // routine NEXT-ONE find following variable
                                          // or position of 0x80 end-marker.
...
# dimensions(1) but since we have yet to double allow for two
        ...
        LD      HL,(0x5C59)               // address E_LINE - first location after
                                          // variables area
        DEC     HL                        // point to location before - the 0x80 end-marker
...
#// NOT-BIN
L2CB8:  CP      0x2E                      // '.' - leading decimal point ?
...
L2CD5:  RST     28H                       //; FP-CALC
        .byte    0xA1                     //;stk-one   ;x or 0,1.
        .byte    0xC0                     //;st-mem-0  ;x or 0,1.
        .byte    0x02                     //;delete    ;x or 0.
        .byte    0x38                     //;end-calc
...
L2CDA:  RST     18H                       // GET-CHAR
        ...
        .byte    0xE0                     //;get-mem-0  ;x or 0,d,1.
        .byte    0xA4                     //;stk-ten    ;x or 0,d,1,10.
        .byte    0x05                     //;division   ;x or 0,d,1/10.
        .byte    0xC0                     //;st-mem-0   ;x or 0,d,1/10.
        .byte    0x04                     //;multiply   ;x or 0,d/10.
        .byte    0x0F                     //;addition   ;x or 0 + d/10.
        .byte    0x38                     //;end-calc   last value.
...
#// SIGN-FLAG
L2CF2:  LD      B,0xFF                    // initialize temporary sign byte to 0xFF
...
L2CFF:  CALL    L2D1B                     // routine NUMERIC
        ...
        INC     B                         // 0xFF to 0x00 or 0x00 to 0x01 - expendable now.
...
#// STACK-BC
L2D2B:  LD      IY,0x5C3A                 // re-initialize ERR_NR
        ...
        RST     28H                       //; FP-CALC
        .byte    0x38                     //;end-calc  make HL = STKEND-5
...
L2D60:  SRL     A                         // 0>76543210>C
        ...
        .byte    0x04                     //;to L2D6D, E-DIVSN

        .byte    0x04                     //;multiply                   x*10.
        .byte    0x33                     //;jump

        .byte    0x02                     //;to L2D6E, E-FETCH

#// E-DIVSN
L2D6D:  .byte    0x05                     //;division                   x/10.

#// E-FETCH
L2D6E:  .byte    0xE1                     //;get-mem-1                  x/10 or x*10, 10.
...
#// p-int-sto
L2D8C:  LD      C,0x00                    // make sign byte positive and continue
...
L2DE3:  RST     28H                       //; FP-CALC
        ...
        .byte    0x0B                     //;to L2DF2, PF-NEGTVE

        .byte    0x31                     //;duplicate
        .byte    0x37                     //;greater-0
        .byte    0x00                     //;jump-true

        .byte    0x0D                     //;to L2DF8, PF-POSTVE
...
#// PF-POSTVE
L2DF8:  .byte    0xA0                     //;stk-zero     x,0.     begin by
...
# A .byte 02 delete is required immediately on using the calculator.

#// PF-SMALL
L2E24:  RST     28H                       //; FP-CALC       int x = 0.
L2E25:  .byte    0xE2                     //;get-mem-2      int x = 0, x-int x.
        .byte    0x38                     //;end-calc
...
# by inserting a '.byte 02 delete' at L2E25, and maintain main addresses.

# the branch was here with a large positive integer > 65535 e.g. 123456789
# the accumulator holds the exponent.

#// PF-LARGE
L2E56:  SUB     0x80                      // make exponent positive
...
L2EA1:  RLD                               // pick up leftmost nibble from (HL)
        ...
        DEC     C                         // test
        INC     C                         // flag
        JR      NZ,L2EB3                  // skip forward to PF-TEST-2 if flag still 0xFF
...
#// PF-E-POS
L2F83:  LD      A,0x2B                    // prepare character '+'
...
#// tbl-addrs
L32D7:  .word    L368F                    // 0x00 Address: 0x368F - jump-true
        .word    L343C                    // 0x01 Address: 0x343C - exchange
        .word    L33A1                    // 0x02 Address: 0x33A1 - delete

# true binary operations.

        .word    L300F                    // 0x03 Address: 0x300F - subtract
        .word    L30CA                    // 0x04 Address: 0x30CA - multiply
        .word    L31AF                    // 0x05 Address: 0x31AF - division
        .word    L3851                    // 0x06 Address: 0x3851 - to-power
        .word    L351B                    // 0x07 Address: 0x351B - or

        .word    L3524                    // 0x08 Address: 0x3524 - no-&-no
        .word    L353B                    // 0x09 Address: 0x353B - no-l-eql
        .word    L353B                    // 0x0A Address: 0x353B - no-gr-eql
        .word    L353B                    // 0x0B Address: 0x353B - nos-neql
        .word    L353B                    // 0x0C Address: 0x353B - no-grtr
        .word    L353B                    // 0x0D Address: 0x353B - no-less
        .word    L353B                    // 0x0E Address: 0x353B - nos-eql
        .word    L3014                    // 0x0F Address: 0x3014 - addition

        .word    L352D                    // 0x10 Address: 0x352D - str-&-no
        .word    L353B                    // 0x11 Address: 0x353B - str-l-eql
        .word    L353B                    // 0x12 Address: 0x353B - str-gr-eql
        .word    L353B                    // 0x13 Address: 0x353B - strs-neql
        .word    L353B                    // 0x14 Address: 0x353B - str-grtr
        .word    L353B                    // 0x15 Address: 0x353B - str-less
        .word    L353B                    // 0x16 Address: 0x353B - strs-eql
        .word    L359C                    // 0x17 Address: 0x359C - strs-add

# unary follow

        .word    L35DE                    // 0x18 Address: 0x35DE - val$
        .word    L34BC                    // 0x19 Address: 0x34BC - usr-$
        .word    L3645                    // 0x1A Address: 0x3645 - read-in
        .word    L346E                    // 0x1B Address: 0x346E - negate

        .word    L3669                    // 0x1C Address: 0x3669 - code
        .word    L35DE                    // 0x1D Address: 0x35DE - val
        .word    L3674                    // 0x1E Address: 0x3674 - len
        .word    L37B5                    // 0x1F Address: 0x37B5 - sin
        .word    L37AA                    // 0x20 Address: 0x37AA - cos
        .word    L37DA                    // 0x21 Address: 0x37DA - tan
        .word    L3833                    // 0x22 Address: 0x3833 - asn
        .word    L3843                    // 0x23 Address: 0x3843 - acs
        .word    L37E2                    // 0x24 Address: 0x37E2 - atn
        .word    L3713                    // 0x25 Address: 0x3713 - ln
        .word    L36C4                    // 0x26 Address: 0x36C4 - exp
        .word    L36AF                    // 0x27 Address: 0x36AF - int
        .word    L384A                    // 0x28 Address: 0x384A - sqr
        .word    L3492                    // 0x29 Address: 0x3492 - sgn
        .word    L346A                    // 0x2A Address: 0x346A - abs
        .word    L34AC                    // 0x2B Address: 0x34AC - peek
        .word    L34A5                    // 0x2C Address: 0x34A5 - in
        .word    L34B3                    // 0x2D Address: 0x34B3 - usr-no
        .word    L361F                    // 0x2E Address: 0x361F - str$
        .word    L35C9                    // 0x2F Address: 0x35C9 - chrs
        .word    L3501                    // 0x30 Address: 0x3501 - not

# end of true unary

        .word    L33C0                    // 0x31 Address: 0x33C0 - duplicate
        .word    L36A0                    // 0x32 Address: 0x36A0 - n-mod-m
        .word    L3686                    // 0x33 Address: 0x3686 - jump
        .word    L33C6                    // 0x34 Address: 0x33C6 - stk-data
        .word    L367A                    // 0x35 Address: 0x367A - dec-jr-nz
        .word    L3506                    // 0x36 Address: 0x3506 - less-0
        .word    L34F9                    // 0x37 Address: 0x34F9 - greater-0
        .word    L369B                    // 0x38 Address: 0x369B - end-calc
        .word    L3783                    // 0x39 Address: 0x3783 - get-argt
        .word    L3214                    // 0x3A Address: 0x3214 - truncate
        .word    L33A2                    // 0x3B Address: 0x33A2 - fp-calc-2
        .word    L2D4F                    // 0x3C Address: 0x2D4F - e-to-fp
        .word    L3297                    // 0x3D Address: 0x3297 - re-stack

# the following are just the next available slots for the 128 compound literals
# which are in range 0x80 - 0xFF.

        .word    L3449                    // 0x3E Address: 0x3449 - series-xx    0x80 - 0x9F.
        .word    L341B                    // 0x3F Address: 0x341B - stk-const-xx 0xA0 - 0xBF.
        .word    L342D                    // 0x40 Address: 0x342D - st-mem-xx    0xC0 - 0xDF.
        .word    L340F                    // 0x41 Address: 0x340F - get-mem-xx   0xE0 - 0xFF.
...
#// RE-ENTRY
L3365:  LD      (0x5C65),DE               // save end of stack in system variable STKEND
        EXX                               // switch to alt
...
L336C:  PUSH    HL                        // save pointer on stack
        ...
        JP      P,L3380                   // forward to FIRST-3D if in range 0x00 - 0x3D
...
#// FIRST-3D
L3380:  CP      0x18                      // compare with first unary operations.
...
#// STACK-NUM
L33B4:  LD      DE,(0x5C65)               // load destination from STKEND system variable.
...
#// FORM-EXP
L33DE:  ADD     A,0x50                    // now add 0x50 to form actual exponent
...
#// LOC-MEM
L3406:  LD      C,A                       // store the original number 0x00-0x1F.
        RLCA                              // double.
        RLCA                              // quadruple.
...
#// exchange
L343C:  LD      B,0x05                    // there are five bytes to be swapped
...
#// series-xx
L3449:  LD      B,A                       // parameter 0x00 - 0x1F to B counter
...
# addresses one of the numbers of the series following the series literal.
        ...
        .byte    0x35                     //;dec-jr-nz
        .byte    0xEE                     //;back to L3453, G-LOOP
...
#// STK-PNTRS
L35BF:  LD      HL,(0x5C65)               // fetch STKEND value from system variable.
...
#// val
#// val$
L35DE:  LD      HL,(0x5C5D)               // fetch value of system variable CH_ADD
...
#// str$
L361F:  LD      BC,0x0001                 // create an initial byte in workspace
...
#// X-NEG
L36B7:  .byte    0x31                     //;duplicate             -3.4, -3.4.
        ...
        .byte    0x03                     //;to L36C2, EXIT        -3.

        .byte    0xA1                     //;stk-one               -3, 1.
        .byte    0x03                     //;subtract              -4.

#// EXIT
L36C2:  .byte    0x38                     //;end-calc              -4.
...
L373D:  .byte    0x01                     //;exchange
        ...
        .byte    0x03                     //;subtract
        .byte    0x8C                     //;series-0C
        .byte    0x11                     //;Exponent: 0x61, Bytes: 1
        .byte    0xAC                     //;(+00,+00,+00)
        .byte    0x14                     //;Exponent: 0x64, Bytes: 1
        .byte    0x09                     //;(+00,+00,+00)
        .byte    0x56                     //;Exponent: 0x66, Bytes: 2
        .byte    0xDA,0xA5                //;(+00,+00)
        .byte    0x59                     //;Exponent: 0x69, Bytes: 2
        .byte    0x30,0xC5                //;(+00,+00)
        .byte    0x5C                     //;Exponent: 0x6C, Bytes: 2
        .byte    0x90,0xAA                //;(+00,+00)
        .byte    0x9E                     //;Exponent: 0x6E, Bytes: 3
        .byte    0x70,0x6F,0x61           //;(+00)
        .byte    0xA1                     //;Exponent: 0x71, Bytes: 3
        .byte    0xCB,0xDA,0x96           //;(+00)
        .byte    0xA4                     //;Exponent: 0x74, Bytes: 3
        .byte    0x31,0x9F,0xB4           //;(+00)
        .byte    0xE7                     //;Exponent: 0x77, Bytes: 4
        .byte    0xA0,0xFE,0x5C,0xFC      //;
        .byte    0xEA                     //;Exponent: 0x7A, Bytes: 4
        .byte    0x1B,0x43,0xCA,0x36      //;
        .byte    0xED                     //;Exponent: 0x7D, Bytes: 4
...
# quadrant III ranges -2 to -1.
        ...
        .byte    0x00                     //;jump-true
        .byte    0x04                     //;to L37A1, ZPLUS  with quadrants II and III.
...
#// YNEG
L37A8:  .byte    0x38                     //;end-calc        quadrants II and III correct.
...
L37AA:  RST     28H                       //; FP-CALC              angle in radians.
        ...
        .byte    0x00                     //;jump-true
        .byte    0x06                     //;fwd to L37B7, C-ENT
                                          //;forward to common code if in QII or QIII.

        .byte    0x1B                     //;negate                else make sign +ve.
        .byte    0x33                     //;jump
        .byte    0x03                     //;fwd to L37B7, C-ENT
...
L37B7:  .byte    0x31                     //;duplicate
        ...
        .byte    0xE6                     //;(+00,+00,+00)
        .byte    0x5C                     //;Exponent: 0x6C, Bytes: 2
        .byte    0x1F,0x0B                //;(+00,+00)
        .byte    0xA3                     //;Exponent: 0x73, Bytes: 3
        .byte    0x8F,0x38,0xEE           //;(+00)
        .byte    0xE9                     //;Exponent: 0x79, Bytes: 4
...
L37E2:  CALL    L3297                     // routine re-stack
        ...
        .byte    0x01                     //;exchange
        .byte    0x00                     //;jump-true
        .byte    0x06                     //;to L37FA, CASES

        .byte    0x1B                     //;negate
        .byte    0x33                     //;jump
        .byte    0x03                     //;to L37FA, CASES
...
L37FA:  .byte    0x01                     //;exchange
        ...
        .byte    0x03                     //;subtract
        .byte    0x8C                     //;series-0C
        .byte    0x10                     //;Exponent: 0x60, Bytes: 1
        .byte    0xB2                     //;(+00,+00,+00)
        .byte    0x13                     //;Exponent: 0x63, Bytes: 1
        .byte    0x0E                     //;(+00,+00,+00)
        .byte    0x55                     //;Exponent: 0x65, Bytes: 2
        .byte    0xE4,0x8D                //;(+00,+00)
        .byte    0x58                     //;Exponent: 0x68, Bytes: 2
        .byte    0x39,0xBC                //;(+00,+00)
        .byte    0x5B                     //;Exponent: 0x6B, Bytes: 2
        .byte    0x98,0xFD                //;(+00,+00)
        .byte    0x9E                     //;Exponent: 0x6E, Bytes: 3
        .byte    0x00,0x36,0x75           //;(+00)
        .byte    0xA0                     //;Exponent: 0x70, Bytes: 3
        .byte    0xDB,0xE8,0xB4           //;(+00)
        .byte    0x63                     //;Exponent: 0x73, Bytes: 2
        .byte    0x42,0xC4                //;(+00,+00)
        .byte    0xE6                     //;Exponent: 0x76, Bytes: 4
        .byte    0xB5,0x09,0x36,0xBE      //;
        .byte    0xE9                     //;Exponent: 0x79, Bytes: 4
        .byte    0x36,0x73,0x1B,0x5D      //;
        .byte    0xEC                     //;Exponent: 0x7C, Bytes: 4
...
L3851:  RST     28H                       //; FP-CALC              X, Y.
        ...
        .byte    0x00                     //;jump-true
        .byte    0x07                     //;to L385D, XISO   if X is zero.
...
#// XISO
L385D:  .byte    0x02                     //;delete                Y.
        .byte    0x31                     //;duplicate             Y, Y.
        .byte    0x30                     //;not                   Y, (1/0).
        .byte    0x00                     //;jump-true
        .byte    0x09                     //;to L386A, ONE         if Y is zero.

        .byte    0xA0                     //;stk-zero              Y, 0.
        .byte    0x01                     //;exchange              0, Y.
        .byte    0x37                     //;greater-0             0, (1/0).
        .byte    0x00                     //;jump-true             0.
        .byte    0x06                     //;to L386C, LAST        if Y was any positive
                                          //;                      number.
...
#// ONE
L386A:  .byte    0x02                     //;delete                .
        .byte    0xA1                     //;stk-one               1.

#// LAST
L386C:  .byte    0x38                     //;end-calc              last value is 1 or 0.
...
#// KEYS
L386E:  PUSH    IX
        BIT     4,(IY+0x01)               // [FLAGS] Test if in 128K mode
...
#// READ_OUTPUTS
L387F:  LD      C,0xFD                    // FFFD = Address of the
...
L38FF:  .byte    0xFF, 0xFF

#// BL_CONTINUE
L3901:  LD      B,0x2B                    // Delay for 564 T-states
        CALL    L38B3
...
L3938:  CALL    L387F                     // Read the output line states
        ...
        LD      A,(0x5B88)                // [FLAGS/ROW3] Has communications already been
        ...
        LD      A,(0x5B88)                // [FLAGS/ROW3] Test if poll counter already zero thus
        ...
        LD      (0x5B89),A                // [ROW2/ROW1]
        JR      L399C                     // Exit the routine

#//POLL_KEYPAD
L395A:  LD      A,(0x5B89)                // [ROW2/ROW1] Decrement the poll counter
        DEC     A
        LD      (0x5B89),A                // [ROW2/ROW1]
...
# The poll counter has reached zero so a poll of the keypad can now occur.
        ...
        XOR     A
        LD      (0x5B88),A                // [FLAGS/ROW3] Indicate that a poll can occur
        LD      (0x5B89),A                // [ROW2/ROW1]
        LD      (0x5B8A),A                // [ROW4/ROW5] Clear all the row nibble stores
        ...
        LD      (0x5B88),A                // [FLAGS/ROW3] Store the flags nibble
        XOR     A
...
L39A0:  CALL    L3938                     // Try to poll the keypad
        ...
        LD      A,(0x5B88)                // [FLAGS/ROW3] Test the flags nibble
        CPL
...
#// KS_UPPER
L39D6:  AND     0x0F                      // Mask off the upper nibble of the nibble store
...
# All five rows have now been read so compose a unique code for the key pressed.
        ...
        LD      IX,0x5B88                 // [FLAGS/ROW3]
        LD      HL,0x3A3F                 // Point to the key mask data
        LD      B,0x03                    // Scan three nibbles

#// GEN_LOOP
L39F0:  LD      A,(IX+0x00)               // Fetch a pair of nibbles
...
L39FF:  .byte    0xFF, 0xFF               // Unused locations
...
#// GEN_POINT
L3A27:  LD      A,(0x5B8A)                // [ROW4/ROW5] Test if the SHIFT key was pressed
        AND     0x08
...
#// KEY_MASKS
L3A3F:  .byte    0x0F, 0xFF, 0xF2         // Key mask data
...
#// KEYPAD
L3A42:  LD      E,0x80                    // Signal no key pressed
        LD      A,(0x5C78)                // [FRAMES]
...
#// KP_CHECK
L3A4F:  LD HL,0x5C00                      // [KSTATE0] Test the first KSTATE variable
...
L3A62:  LD      A,L                       // Jump back and test the second set if
        LD      HL,0x5C04                 // [KSTATE4] not yet considered
        CP      L
        JR      NZ,L3A52
        ...
        LD      HL,0x5C00                 // [KSTATE0] repeated
        CP      (HL)
        JR      Z,L3A9E                   // Jump if being repeated

        EX      DE,HL                     // Save the address of KSTATE0
        LD      HL,0x5C04                 // [KSTATE4] Test if the key in the second set is
...
L3A83:  LD      E,A                       // Pass the key code to the E register
        ...
        LD      A,(0x5C09)                // [REPDEL] Fetch the initial repeat delay
...
#// KP_END
L3A94:  LD      A,E
        LD      (0x5C08),A                // [LAST_K] Store the key value in LAST_K
        LD      HL,0x5C3B                 // FLAGS
...
L3A9E:  INC     HL
        ...
        LD      A,(0x5C0A)                // [REPPER] The subsequent repeat delay is
...
#// KP_TEST
L3AAE:  LD      A,E
        LD      HL,0x5B66                 // FLAGS3 Test if in BASIC or EDIT mode
        BIT     0,(HL)
...
L3AD7:  PUSH    HL                        // Save the KSTATE pointer
        ...
        LD      HL,0x5B66                 // FLAGS3 Test if in EDIT or BASIC mode
        BIT     0,(HL)
...
L3AFE:  .byte    0x00, 0xFF, 0xFF         // Unused locations

#// KPD_CONT
L3B01:  CP      0x17                      // Test for SHIFT 3
        JR      Z,L3B0F
...
#// KPD_TABLE1
L3B13:  .byte    0x2E, 0x0D, 0x33         // '.', ENTER, 3
        .byte    0x32, 0x31               // 2, 1

#// KPD_TABLE2
L3B18:  .byte    0x29, 0x28, 0x2A         // ), (, *
        .byte    0x2F, 0x2D, 0x39         // /, - , 9
        .byte    0x38, 0x37, 0x2B         // 8, 7, +

#// KPD_TABLE3
L3B21:  .byte    0x36, 0x35, 0x34         // 6, 5, 4
        .byte    0x30                     // 0

#// KPD_TABLE4
L3B25:  .byte    0xA5, 0x0D, 0xA6         // Bottom, ENTER, Top
...
L3B3B:  BIT     4,(IY+0x01)               // FLAGS 3 - In 128K mode?
...
L3B46:  LD      HL,0x010F                 // Vector table entry in Editor ROM -> JP 0x03A2

# Return to Editor ROM at address in HL

L3B49:  EX      (SP),HL                   // Change the return address
        JP      0x5B00                    // Page Editor ROM and return to the address on the stack

# -------------------------------------
# STATEMENT INTERPRETATION RETURN PATCH
# -------------------------------------

L3B4D:  BIT     4,(IY+0x01)               // In 128K mode?
        JR      NZ,L3B58                  // NZ=128K mode

# In 48K mode

        BIT     7,(IY+0x0A)               // replicate code from standard ROM that the patch over-wrote
        RET

# In 128K mode

L3B58:  LD      HL,0x0112                 // Handle in Editor ROM by jumping to Vector table entry in Editor ROM -> JP #182A
        JR      L3B49
...
L3B5D:  BIT     4,(IY+0x01)               // In 128K mode?
...
L3B67:  LD      HL,0x0115                 // Handle in Editor ROM by jumping to Vector table entry in Editor ROM -> JP #18A8
        JR      L3B49
...
L3BE1:  .byte    0x00, 0x00, 0x00         // Unused locations
        .byte    0x00, 0x00, 0x00
        .byte    0x00, 0x00, 0x00
        .byte    0x00, 0x00, 0x00
        .byte    0x00, 0x00, 0x00
...
#// TV_TUNER
L3C10:  LD      A,0x7F                    // Test for the BREAK key
        IN      A,(0xFE)
        RRA
        ...
        XOR     A
        LD      (0x5C3C),A                // [TV_FLAG] Signal using main screen
...
L3C34:  LD      A,B                       // Calculate the paper colour
        ...
        DEC     A
        LD      (0x5C8F),A                // [ATTR_T] Store as temporary attribute value
...
L3C45:  LD      A,(HL)                    // Fetch a character from the data
        ...
        LD      HL,0x4800                 // Point to 2nd third of display file
        LD      D,H
        LD      E,L
        ...
        LD      DE,0x5900                 // Point to 2nd third of attributes file
        LD      BC,0x0200
...
#// TVT_TONE
L3C68:  LD      DE,0x0370                 // DE=twice the tone frequency in Hz
        LD      L,0x07                    // Border colour of white

#// TVT_DURATION
L3C6D:  LD      BC,0x0099                 // Delay for 950.4us
...
#// TVT_DATA
L3C8F:  .byte    0x13, 0x00               // Bright, off
        .byte    0x31, 0x39               // '1', '9'
...
# L3C97:  .byte    0x00, 0x00, 0x00       // Unused locations
#         .byte    0x00, 0x00, 0x00
#         .byte    0x00, 0x00, 0x00
#         .byte    0x00, 0x00, 0x00
#         .byte    0x00, 0x00, 0x00
